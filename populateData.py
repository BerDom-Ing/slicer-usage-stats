...
# Function to get location from IP using GeoIP database
...
for module in modules:
    num_functions = random.randint(1, 500)  # Random number of functions (1 to 10)
    for _ in range(num_functions):
        ip = generate_random_ip()
        country, country_code, city = get_location_from_ip(ip, db_path)
        country_code_alpha3 = convert_alpha2_to_alpha3(country_code)  # Convert to alpha3
        data.append({
            "function": random.choice(functions),
            "time": random_datetime(start_time, end_time),
            "selected_module": module,
            "country": country,
            "country_code": country_code_alpha3,
            "city": city
        })
